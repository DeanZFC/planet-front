<template>
  <div class="router-history">
    <el-tabs
      v-model="activeValue"
      :closable="!(histories.length === 1 && $route.name === defaultRouter)"
      type="card"
...
const openContextMenu = (e) => {
  if (histories.value.length === 1 && route.name === defaultRouter.value) {
    return false;
  }
  let id = "";
...
const closeLeft = () => {
  ...
  const activeIndex = histories.value.findIndex(
...
const initPage = () => {
  ...
  histories.value =
    JSON.parse(sessionStorage.getItem("histories")) || initHistories;
